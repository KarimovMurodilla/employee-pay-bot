...
def downgrade() -> None:
    ...
    sa.ForeignKeyConstraint(['updated_by'], ['users.id'], name='fk_settings_updated_by_users'),
    ...
    sa.Column('amount_change', sa.NUMERIC(precision=15, scale=2), autoincrement=False, nullable=False),
    sa.Column('balance_before', sa.NUMERIC(precision=15, scale=2), autoincrement=False, nullable=False),
    sa.Column('balance_after', sa.NUMERIC(precision=15, scale=2), autoincrement=False, nullable=False),
    sa.Column('description', sa.TEXT(), autoincrement=False, nullable=True),
    sa.Column('created_by', sa.BIGINT(), autoincrement=False, nullable=True),
    sa.Column('created_at', postgresql.TIMESTAMP(), autoincrement=False, nullable=False),
    sa.ForeignKeyConstraint(['created_by'], ['users.id'], name='fk_balance_history_created_by_users'),
    sa.ForeignKeyConstraint(['transaction_id'], ['transactions.id'], name='fk_balance_history_transaction_id_transactions'),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], name='fk_balance_history_user_id_users'),
    sa.PrimaryKeyConstraint('id', name='pk_balance_history')
    )
    op.create_index('idx_balance_history_user_id', 'balance_history', ['user_id'], unique=False)
    op.create_index('idx_balance_history_created_at', 'balance_history', ['created_at'], unique=False)
    ...
    sa.Column('created_at', postgresql.TIMESTAMP(), autoincrement=False, nullable=False),
    sa.ForeignKeyConstraint(['recipient_id'], ['users.id'], name='fk_notifications_recipient_id_users'),
    sa.ForeignKeyConstraint(['transaction_id'], ['transactions.id'], name='fk_notifications_transaction_id_transactions'),
    sa.PrimaryKeyConstraint('id', name='pk_notifications')
    )
    op.create_index('idx_notifications_recipient', 'notifications', ['recipient_id'], unique=False)
